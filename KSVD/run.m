...
img =imread('Test_Fig2_Missing.png');  %uint8    

Case = 2;
switch Case
    %% Interpolation 普通插值复原法
    case 1 
        Mask = double(~(img(:,:,1)==0)); %判断是否为黑，即空缺点，相当于是一个flg矩阵
        for channel = 1:3
            IMin0 = img(:,:,channel); %分离通道，IMin0为当前通道图片内容
            IMin0 = double(IMin0); % 由uint8转为double
